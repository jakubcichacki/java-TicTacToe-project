...
public class Game {
    ...
    public int checkColumn (char [][] grid) {
        int winningColumn = 3;
        for (int i = 0; i < 3; i++) {
            if (grid[i][0] == grid[i][1] && grid[i][0] != '-') {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        int winColumn = checkColumn(grid);
        if (winColumn != 3) {
            if(grid[winColumn][0] == 'x')
                result = "X wins";
            else result = "O wins";
        }

        int winRow = checkRow(grid);
        if (winRow != 3) {
            if(grid[0][winRow] == 'x')
                result = "X wins";
            else result = "O wins";
